...
const SESSION_LONG_HELP: &str = "Examples:\n  todui session list\n  todui session new \"Writing Sprint\" --tag work --repo @exampleorg/todui-keymove\n  todui session history writing-sprint\n  todui session tag writing-sprint --set private\n  todui session repo writing-sprint --set https://github.com/ExampleOrg/todui-keymove";
    ...
    fn parses_repo_commands_and_repo_flags() {
        ...
        match cli.command.expect("command") {
            Command::Session {
                ...
                assert_eq!(repo.as_deref(), Some("@ExampleOrg/todui-keymove"));
            }
            other => panic!("unexpected command: {other:?}"),
        }
